...
def test_binary_search_one_symbol():
    ...
    for string, sub_string, case_sensitivity, method, count, expected in TEST_SEARCH_ONE_SYMBOL:
        get = search.search(string, sub_string, case_sensitivity, method,
                            count)
        print(
            f"get: {get} | expected: {expected} {'!!!' if get != expected else ''}")
...
def test_binary_search_many_symbol():
    ...
    for string, sub_string, case_sensitivity, method, count, expected in TEST_SEARCH_MANY_SYMBOL:
        get = search.search(string, sub_string, case_sensitivity, method,
                            count)
        print(
            f"str: {string}; substr: {sub_string}; case_sense: {case_sensitivity}; method: {method}; count: {count}  "
